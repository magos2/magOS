...
class byte_code {
  ...
  int op_code;
  string mnemonic;
  int operand_size; //operand size in bytes
};
...
class InstructionSet
{
  public:
    ...
    string get_mnemonic (int op_code);
    int get_operand_size (int op_code);
    bool is_imm_oprand (int op_code); // return true if op_code has immediate operand(not in constant pool)
    
  private:
    static InstructionSet* _instance;
    InstructionSet ();
};
extern byte_code byte_code_table[];

#define   def_op(op_code, mnemonic, operand_size) {(op_code), (mnemonic), (operand_size)}
